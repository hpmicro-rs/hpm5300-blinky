...
use hpm_metapac::{self as pac, gpiom::vals, FGPIO, IOC, SYSCTL};
...
fn main() -> ! {
    ...
    FGPIO.oe(PA).clear().write(|w| w.set_direction(1 << 3));

    IOC.pad(pac::pins::PA03).pad_ctl().modify(|w| {
        w.set_prs(0b11);
        w.set_ps(false);
        w.set_pe(true);
    });
    ...
    loop {
        let btn = FGPIO.di(PA).value().read().0 & (1 << 3) != 0;

        defmt::info!("tick button={}", btn);
